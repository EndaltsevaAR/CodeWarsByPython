...
def is_interesting(number, awesome_phrases):
    if exactly_interest(number, awesome_phrases):
        return 2
    elif exactly_interest(number + 1, awesome_phrases) or exactly_interest(number + 2, awesome_phrases):
        return 1
    else:
        return 0


def exactly_interest(number, awesome_phrases):
    if number < 100 or number > 1000000000:  # small or big numbers
        return False
    if number in awesome_phrases:  # interesting numbers
        return True
    n_str = str(number)
    if is_all_zeroes(n_str) or is_the_same(n_str) or is_increment(n_str) or is_decrement(n_str) or is_palindrome(n_str):
        return True
    return False


def is_all_zeroes(number):
    if number[0:1:] == '0':
        return False
...
def is_palindrome(number):
    if number[0:math.ceil(len(number)/2)-1:] == "".join(reversed(number[math.ceil(len(number)/2)::])):
        return True


print(is_interesting(101, [1337, 256]))
