...
def mix(s1, s2):
    ...
    for letter in alphabet:
        s1_char = s1.count(letter) * letter
        s2_char = s2.count(letter) * letter

        s1_length = len(s1_char)
        s2_length = len(s2_char)
        if s1_length > 1 or s2_length > 1:
            if s1_length == s2_length:
                final_list.append("=:" + s1_char)
            elif s1_length > s2_length:
                final_list.append("1:" + s1_char)
            else:
                final_list.append("2:" + s2_char)
    final_list.sort(key=lambda x: (-len(x), x[0]))
